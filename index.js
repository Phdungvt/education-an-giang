...
const batchInsertImagesBtn = document.getElementById('batch-insert-images-btn');
...
function renderQuiz() {
    ...
    if (quizData.length === 0) {
        ...
        sharedActionsContainer.classList.add('hidden');
        imageUploadContainer.classList.add('hidden');
        return;
    }
    placeholder.classList.add('hidden');
    sharedActionsContainer.classList.remove('hidden');
    ...
    const renderGroup = (title, questions) => {
        if (!questions || questions.length === 0) return;

        const groupHeader = document.createElement('h2');
        groupHeader.className = "text-xl font-bold text-gray-800 border-b-2 pb-2 mb-4";
        ...
        questions.forEach((item) => {
            ...
            questionEl.className = 'question-card p-5 border border-gray-200 rounded-xl bg-white shadow-sm';
            questionEl.dataset.index = String(globalIndex);

            let headerHTML = `
                <div class="flex justify-between items-start">
                    <div class="flex-grow pr-4">
                        <h3 class="font-bold text-lg mb-2 text-gray-800">Câu ${item.questionNumber || (globalIndex + 1)}: ${item.isDirty ? '<span class="text-yellow-500 font-bold" title="Các thay đổi chưa được định dạng bởi AI">*</span>' : ''}</h3>
                    </div>
                    <button data-index="${globalIndex}" class="edit-btn ml-4 flex-shrink-0 px-3 py-1.5 bg-yellow-400 text-yellow-900 text-sm font-semibold rounded-md hover:bg-yellow-500 transition-colors">Sửa</button>
                </div>`;

            let bodyHTML = '';
            const questionText = item.type === 'true-false' ? item.main_question : item.question;
            bodyHTML += `<div class="question-content text-gray-700">${questionText}</div>`;
                    ...
                    bodyHTML += `
                        <div class="options-grid mt-4 grid grid-cols-1 md:grid-cols-2 gap-3">
                            ${(item.options || []).map((opt, i) => {
                                ...
                                return `
                                <div class="flex items-start gap-x-2 p-3 border rounded-lg ${i === item.correctAnswerIndex ? 'border-green-500 bg-green-50' : 'border-gray-300 bg-gray-50'}">
                                    <span class="font-bold flex-shrink-0">${String.fromCharCode(65 + i)}.</span>
                                    <div class="option-content w-full">${opt}${optionImageHTML}</div>
                                </div>`
                            }).join('')}
                        </div>`;
                    break;
    ...
    renderGroup('PHẦN I: TRẮC NGHIỆM NHIỀU LỰA CHỌN', groupedQuestions['multiple-choice']);
    renderGroup('PHẦN II: ĐÚNG/SAI', groupedQuestions['true-false']);
    renderGroup('PHẦN III: TRẢ LỜI NGẮN', groupedQuestions['short-answer']);
    renderGroup('PHẦN IV: TỰ LUẬN', groupedQuestions['essay']);
...
async function generateQuizFromBatch() {
    ...
    if (!knowledgeBase) {
        alert('Vui lòng nạp một tệp kiến thức trước khi tạo bài tập.');
        return;
    }
    ...
    const generationPromises = requestBatch.map(async (request) => {
        let systemPrompt = `Bạn là một trợ lý AI chuyên tạo đề thi trắc nghiệm toán học. Dựa vào nội dung tài liệu được cung cấp, hãy tạo ra chính xác ${request.count} câu hỏi về chủ đề "${request.prompt}".
QUAN TRỌNG: 
1. Tất cả các công thức toán học PHẢI được viết bằng mã LaTeX. Sử dụng $...$ cho công thức inline và $$...$$ cho công thức hiển thị riêng dòng.
2. Nếu một câu hỏi yêu cầu hình vẽ, đồ thị hoặc bảng biến thiên, TUYỆT ĐỐI KHÔNG tự vẽ. Thay vào đó, hãy chèn một placeholder văn bản với định dạng: [HÌNH VẼ: Đồ thị của hàm số y = f(x) CỤ THỂ] hoặc [BẢNG BIẾN THIÊN: Bảng biến thiên của hàm số y = f(x) CỤ THỂ]. Ví dụ: [HÌNH VẼ: Đồ thị của hàm số y = x^3 - 3x + 2]. Mô tả phải rõ ràng và đầy đủ để giáo viên có thể vẽ lại chính xác.`;
        if (request.difficulty !== 'Không yêu cầu') {
            systemPrompt += ` Các câu hỏi phải ở mức độ ${request.difficulty}.`;
        }
...
---
YÊU CẦU ĐỊNH DẠNG:
Mỗi câu hỏi phải có một phát biểu chung và 4 mệnh đề con. Trả về một mảng JSON theo cấu trúc sau:
[{"main_question": "...", "statements": [{"statement": "...", "is_correct": true/false}, ... ]}]
---
NỘI DUNG TÀI LIỆU:
"${knowledgeBase}"`;
                schema = { type: 'ARRAY', items: { type: 'OBJECT', properties: { "main_question": { "type": 'STRING' }, "statements": { "type": 'ARRAY', "items": { "type": 'OBJECT', properties: { "statement": { "type": 'STRING' }, "is_correct": { "type": 'BOOLEAN' } }, required: ["statement", "is_correct"] } } }, required: ["main_question", "statements"] } };
                break;
            case 'short-answer':
                finalPrompt = `${systemPrompt}
---
YÊU CẦU ĐỊNH DẠNG:
Các câu hỏi này yêu cầu tính toán và điền một con số hoặc giá trị cụ thể. Trả về một mảng JSON theo cấu trúc sau:
[{"question": "...", "answer": "..."}]
---
NỘI DUNG TÀI LIỆU:
"${knowledgeBase}"`;
...
---
YÊU CẦU ĐỊNH DẠNG:
Các câu hỏi này là câu hỏi tự luận, yêu cầu giải thích hoặc chứng minh. Trả về một mảng JSON theo cấu trúc sau:
...
---
YÊU CẦU ĐỊNH DẠNG:
Trả về một mảng JSON có cấu trúc: [{"question": "...", "options": ["A", "B", "C", "D"], "correctAnswerIndex": 0-3}].
Quan trọng: Các chuỗi đáp án trong mảng 'options' không được chứa tiền tố như 'A.', 'B.', v.v... và phải sử dụng LaTeX.
---
NỘI DUNG TÀI LIỆU:
"${knowledgeBase}"`;
                schema = { type: 'ARRAY', items: { type: 'OBJECT', properties: { "question": { "type": 'STRING' }, "options": { "type": 'ARRAY', "items": { "type": 'STRING' } }, "correctAnswerIndex": { "type": 'INTEGER' } }, required: ["question", "options", "correctAnswerIndex"] } };
                break;
        }
        ...
        results.forEach((result) => {
            if (result.text) {
                try {
                    ...
                    const typedData = parsedData.map((item) => ({ ...item, type: result.requestType, questionNumber: combinedQuizData.length + 1 }));
...
async function formatTextWithLatex(text) {
    ...
    const prompt = `Chuyển đổi văn bản sau để công thức toán học dùng chỉ inline LaTeX $...$ và display LaTeX $$...$$ (tuyệt đối không chèn newline trong công thức để không nhảy dòng). Nếu có mã TikZ, hãy giữ nguyên. Chỉ trả về chuỗi đã định dạng. Văn bản: "${text}"`;
...
function renderRequestBatch() {
    ...
    if (requestBatch.length > 0) {
        ...
        list.innerHTML = `<h4 class="text-sm font-semibold text-gray-800 border-t pt-3">Danh sách yêu cầu:</h4>`;
        requestBatch.forEach((req, index) => {
            ...
            item.innerHTML = `
                <span class="flex-grow">${req.count} câu ${typeText} (${req.difficulty})</span>
                <button data-index="${index}" class="remove-request-btn text-red-500 hover:text-red-700">
    ...
    <body class="bg-slate-100 text-slate-800">
        ...
        <script>
            ...
            function renderStudentQuiz() {
                ...
                const groupTitles = {
                    'multiple-choice': 'PHẦN I: TRẮC NGHIỆM NHIỀU LỰA CHỌN',
                ...
                const renderGroup = (title, questions) => {
                    ...
                    let groupHTML = \`<div class="group-container space-y-8">\`;
                    groupHTML += \`<h2 class="text-2xl font-bold text-slate-800 border-b-2 border-slate-300 pb-3 mb-6">\${title}</h2>\`;
                    ...
                    questions.forEach((item, localIndex) => {
                        ...
                        const currentQuestionNumber = (groupedQuestions['multiple-choice']?.indexOf(item) ?? -1) !== -1 ? (groupedQuestions['multiple-choice'].indexOf(item) + 1) :
                                                   (groupedQuestions['true-false']?.indexOf(item) ?? -1) !== -1 ? (groupedQuestions['multiple-choice']?.length ?? 0) + (groupedQuestions['true-false'].indexOf(item) + 1) :
                                                   (groupedQuestions['short-answer']?.indexOf(item) ?? -1) !== -1 ? (groupedQuestions['multiple-choice']?.length ?? 0) + (groupedQuestions['true-false']?.length ?? 0) + (groupedQuestions['short-answer'].indexOf(item) + 1) :
                                                   (groupedQuestions['essay']?.indexOf(item) ?? -1) !== -1 ? (groupedQuestions['multiple-choice']?.length ?? 0) + (groupedQuestions['true-false']?.length ?? 0) + (groupedQuestions['short-answer']?.length ?? 0) + (groupedQuestions['essay'].indexOf(item) + 1) :
                                                   (globalIndex + 1);
                        ...
                        groupHTML += \`<div class="flex items-start">
                                     <div class="flex-grow">
                                         <div class="flex items-center mb-2">
                                             <span class="font-bold text-lg mr-3 text-indigo-600">Câu \${currentQuestionNumber}:</span>
                ...
                renderGroup(groupTitles['multiple-choice'], groupedQuestions['multiple-choice']);
                renderGroup(groupTitles['true-false'], groupedQuestions['true-false']);
                renderGroup(groupTitles['short-answer'], groupedQuestions['short-answer']);
                renderGroup(groupTitles['essay'], groupedQuestions['essay']);
            ...
            function handleSubmit() {
                        ...
                        revealAnswerBtn.onclick = () => {
                            ...
                            switch(item.type) {
                                ...
                                case 'essay':
                                    ...
                                    feedbackEl.appendChild(answerContainer);
                                    if (window.MathJax) {
                ...
                if (retriesLeft > 0 || quizOptions.retriesAllowed === 0) {
            ...
            function startNewAttempt() {
                ...
                if(quizOptions.retriesAllowed > 0) {
                     retriesLeft--;
            ...
            studentInfoForm.addEventListener('submit', (e) => {
                ...
                if (quizOptions.retriesAllowed > 0) {
                    ...
                    retriesDisplay.textContent = 'Không giới hạn lượt làm lại';
                }
...
async function handleExplainClick(event) {
    ...
    if (!isHidden && !explanationContent.innerHTML) { // Only fetch if it's being shown and is empty
        ...
        if (!item.explanation) { // Only call API if explanation is not already fetched
            const questionText = item.type === 'true-false' ? item.main_question : item.question;
            let promptText = `Hãy cung cấp một lời giải chi tiết, từng bước một cho câu hỏi toán sau đây. Giải thích rõ ràng các khái niệm và công thức được sử dụng.\n---
CÂU HỎI:\n${questionText}\n---
`;
            if (item.type === 'multiple-choice') {
        ...
        window.MathJax.typesetPromise([explanationContent]);
...
async function handleVaryClick(event) {
    ...
    const promptText = `Dựa vào câu hỏi toán học sau đây, hãy tạo ra MỘT câu hỏi MỚI có nội dung tương tự nhưng với các số liệu và ngữ cảnh khác.
---
...
function resetToWorkflowChoice() {
    ...
    requestBatch = [];
    knowledgeBase = "";
    renderQuiz(); // This will show the placeholder
...
async function handleFinalizeEdits() {
    ...
    try {
        const formatPromises = dirtyQuestions.map(async ({ q, i }) => {
            ...
            } else if (formattedQuestion.type === 'short-answer' || formattedQuestion.type === 'essay') {
...
${quizContent}`;
    ...
    try {
        ...
        const content = response.text || "Không thể tạo đề cương.";
        ...
        let htmlContent = content
            ...
            .replace(/(\n|^)## (.*)/g, '$1<h2 class="text-xl font-bold mt-6 mb-3 border-b pb-2">$2</h2>')
            .replace(/(\n|^)# (.*)/g, '$1<h1 class="text-2xl font-bold mt-8 mb-4 border-b-2 pb-2">$2</h1>')
            .replace(/\n\s*-\s/g, '</li><li class="ml-5 list-disc">') // list items
            .replace(/<\/li><li/g, '</li></ul><ul class="space-y-1"><li') // fix first item
            .replace(/\n/g, '<br>');

        studyGuideContent.innerHTML = htmlContent;
        window.MathJax.typesetPromise([studyGuideContent]);
...
async function handleBatchImageInsert(event) {
    const input = event.target;
    ...
    if (imageFiles.length === 0) {
        alert('Không tìm thấy tệp hình ảnh nào trong thư mục đã chọn.');
        globalLoader.style.display = 'none';
        return;
    }
    ...
    const processingPromises = imageFiles.map(async file => {
        const match = file.name.match(/cau_(\d+)/i);
        if (match && match[1]) {
            const questionNumber = parseInt(match[1], 10);
            const targetQuestion = quizData.find(q => q.questionNumber === questionNumber);

            if (targetQuestion) {
                try {
                    const base64Image = await readFileAsBase64(file);
                    return { question: targetQuestion, image: base64Image };
                } catch (error) {
                    warnings.push(`Lỗi đọc tệp: ${file.name}`);
                    return null;
                }
            } else {
                warnings.push(`Không tìm thấy câu hỏi số '${questionNumber}' (từ tệp '${file.name}').`);
                return null;
            }
        } else {
            warnings.push(`Tên tệp '${file.name}' không theo định dạng 'cau_X...'.`);
            return null;
        }
    });
    ...
    try {
        ...
        results.forEach(result => {
            if (result) {
                result.question.questionImage = result.image;
                result.question.isDirty = true;
                insertedCount++;
            }
        });
        
        let alertMessage = `Đã chèn thành công ${insertedCount} hình ảnh.`;
        if (warnings.length > 0) {
            alertMessage += `\n\nCó ${warnings.length} cảnh báo:\n- ${warnings.join('\n- ')}`;
            console.warn("Cảnh báo khi chèn ảnh (thông minh):", warnings);
        }
        alert(alertMessage);
        renderQuiz();
    } catch (error) {
        console.error("Lỗi khi xử lý hình ảnh (thông minh):", error);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    addRequestBtn.addEventListener('click', () => {
        ...
        if (!userPrompt) {
            alert('Vui lòng nhập nội dung yêu cầu chính.');
            return;
        }
        requestBatch.push({
    ...
    batchInsertImagesBtn.addEventListener('click', () => imageFolderInput.click());

    imageFolderInput.addEventListener('change', handleBatchImageInsert);
    ...
    editPasteImageBtn.addEventListener('click', async () => {
        try {
            ...
            alert('Không thể dán ảnh. Vui lòng kiểm tra quyền truy cập clipboard.');
        }
    });
